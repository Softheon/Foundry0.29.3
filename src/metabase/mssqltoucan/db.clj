...
(def ^:dynamic *db-connection*
  "Bind this to override the default DB connection used by `metabase.mssqltoucan.db` functions. Provided for situations where you'd
...
(defn set-default-db-connection!
  ...
  the various `metabase.mssqltoucan.db` functions.
...
(defn resolve-model
  "Resolve a model *if* it's quoted. This also unwraps entities when they're inside vectores.
     ...
     (resolve-model 'Database)        -> #'my-project.models.database/Database"
  [model]
  {:post [(:metabase.mssqltoucan.models/model %)]}
  (cond
    (:metabase.mssqltoucan.models/model model) model
...
(defn- replace-boolean-with-number
