(ns metabase.mssqltoucan.models
...
(defn- invoke-model
  ...
  ([model]
   ((resolve 'metabase.mssqltoucan.db/select) model))
  ([model id]
   (when id
...
(defmacro defmodel
  ...
  the model or instances fetched from the DB.
     ...
     (db/select User, ...)  ; use with `metabase.mssqltoucan.db` functions. All results are instances of `UserInstance`
